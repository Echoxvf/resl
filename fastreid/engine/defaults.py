...
__all__ = ["default_argument_parser", "default_setup", "DefaultPredictor", "DefaultTrainer","UnsupervisedTrainer"]
...
class DefaultTrainer(TrainerBase):
    ...
    def __init__(self, cfg):
        ...
        self._trainer = (AMPTrainer if cfg.SOLVER.AMP.ENABLED else SimpleTrainer)(
            model, data_loader, optimizer, cfg, param_wrapper
        )
    ...
    @classmethod
    def build_train_loader(cls, cfg, clustering=False):
        """
        Returns:
            iterable
        It now calls :func:`fastreid.data.build_reid_train_loader`.
        ...
        logger = logging.getLogger(__name__)
        logger.info("Prepare training set")
        return build_reid_train_loader(cfg, combineall=cfg.DATASETS.COMBINEALL, clustering = clustering)
...
class UnsupervisedTrainer(TrainerBase):
    ...
    def __init__(self, cfg):
        ...
        # for param, param_m in zip(model.parameters(), self.model_ema.parameters()):
    ...
    def build_hooks(self):
        ...
        ret.append(hooks.EvalHook(cfg.TEST.EVAL_PERIOD, test_and_save_results))
        ret.append(hooks.Clustering(cfg, self.model, self.cluster_loader))
    ...
    def test(cls, cfg, model):
        ...
        for idx, dataset_name in enumerate(cfg.DATASETS.TESTS):
            ...
            try:
                ...
                results[dataset_name] = {}
                continue
            results_i = inference_on_dataset(model, data_loader, evaluator, flip_test=cfg.TEST.FLIP.ENABLED, ema=cfg.MODEL.EMA)
            results[dataset_name] = results_i
            if comm.is_main_process():
                assert isinstance(
                    results, dict
...
for _attr in ["model", "data_loader", "optimizer", "grad_scaler"]:
    setattr(UnsupervisedTrainer, _attr, property(lambda self, x=_attr: getattr(self._trainer, x, None)))
