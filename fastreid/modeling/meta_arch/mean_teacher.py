...
from fastreid.modeling.meta_arch import META_ARCH_REGISTRY, build_model, Baseline, BaselineUn
from fastreid.utils.checkpoint import Checkpointer
logger = logging.getLogger(__name__)

@META_ARCH_REGISTRY.register()
class MeanTeacher(BaselineUn):
    def __init__(self, cfg):
        cfg.defrost()
        cfg.MODEL.META_ARCHITECTURE = "BaselineUn"
        super().__init__(cfg)
        ...
        #         Checkpointer(model_self).load(cfg.MODEL.WEIGHTS)
        #     else:
    ...
    def _momentum_update_key_encoder(self, m=0.999):
        ...
        """
        # print(len(self.parameters()), len(self.model_t.parameters()))
        for param_q, param_k in zip(self.parameters(), self.model_t.parameters()):
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            indexs = batched_inputs["indexs"]#.to(self.device)
            ...
            with torch.no_grad():
                ...
                # for model_t in self.model_ts:
                    t_feat = self.model_t.backbone(images)
                    t_output = self.model_t.heads(t_feat) #, targets
                    # t_outputs.append(t_output)
            losses = self.losses(s_outputs, indexs, targets)
        ...
        # Eval mode, just conventional reid feature extraction
            ...
            t_feat = self.model_t.backbone(images)
            t_output =  self.model_t.heads(t_feat)
            return t_output #super().forward(batched_inputs)

    def losses(self, s_outputs, indexs, gt_labels):
        """
        Compute loss from modeling's outputs, the loss function input arguments
        must be the same as the outputs of the model forwarding.
        """
        loss_dict = super().losses(s_outputs, gt_labels,indexs)
