...
def inference_on_dataset(model, data_loader, evaluator, flip_test=False, ema=False):
    """
...
def extract_on_dataset(model, data_loader, flip_test=False):
    ...
    with inference_context(model), torch.no_grad():
        for idx, inputs in enumerate(data_loader):
            if idx == num_warmup:
                ...
                total_compute_time = 0
            start_compute_time = time.perf_counter()
            outputs = model(inputs, clustering=True)#.detach()
            # Flip test
            if flip_test:
                inputs["images"] = inputs["images"].flip(dims=[3])
                flip_outputs = model(inputs)
            ...
            for pid, output1,output2, output3, output4 in zip(inputs['indexs'],  outputs[0],outputs[1],outputs[2],outputs[3]):
                features.append(torch.stack([output1,output2, output3, output4])) 
                indexs.append(pid.cpu().item()) 
    ...
    # features = torch.stack(features).clone().cpu()
