...
class MSMT17(ImageDataset):
    ...
    def __init__(self, root='datasets', **kwargs):
        ...
        self.dataset_dir = osp.join(root, "MSMT17_V1")
        self.train_dir   = osp.join(self.dataset_dir, 'bounding_box_train')
        self.query_dir   = osp.join(self.dataset_dir, 'query')
        self.gallery_dir = osp.join(self.dataset_dir, 'bounding_box_test')
        self.cam_dir     = osp.join(self.dataset_dir, 'bounding_box_train_camstyle')
        self.cam_style = defaultdict(list)
        self._get_camstyle_img(self.cam_dir)
        required_files = [
            self.dataset_dir ,
            self.train_dir   ,
            self.query_dir   ,
    ...
    def _get_camstyle_img(self, cam_dir):
        for img in os.listdir(cam_dir):
            self.cam_style[img.split("_fake")[0]].append(os.path.join(cam_dir, img))
