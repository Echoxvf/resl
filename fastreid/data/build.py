...
from .common import CommDataset,CommMBDataset
...
def _train_loader_from_config(cfg, clustering,*, train_set=None, transforms=None, sampler=None, pseudo_labels=None, **kwargs):
    ...
    if train_set is None:
        ...
        if clustering:
            train_set = CommDataset(train_items, transforms, relabel=True)
            train_set.cam_images=data.cam_style
        # else:
            # train_set = CommMBDataset(train_items, transforms,data.cam_style, relabel=True)
    ...
    if sampler is None:
        ...
        elif sampler_name == "RandomMultipleGallerySampler":
            sampler = samplers.RandomMultipleGallerySampler(train_set.img_items, mini_batch_size, num_instance)
        else:
...
def build_reid_train_loader(
        train_set, *, sampler=None, total_batch_size, num_workers=0, is_clustering=False,pseudo_labels=None
):
    """
...
def build_reid_unsupervised_train_loader(cfg,  train_set=None, transforms=None, sampler=None, pseudo_labels=None, reliabilities = None, cam_images = None, **kwargs):
    logger = logging.getLogger(__name__)
    if transforms is None:
        transforms = build_transforms(cfg, is_train=True)
    if train_set is None:
        ...
        for d in cfg.DATASETS.NAMES:
            ...
            train_items.extend(data.train)
        pseudo_items = list()
        train_reliability = list()
        for item, pseudo_label, reliability in zip(train_items, pseudo_labels, reliabilities):
            if pseudo_label ==-1 : continue
            item = list(item)
            item[1] = pseudo_label
            item = tuple(item)
            pseudo_items.append(item)
            train_reliability.append(reliability)
        pseudoDataset = PseudoDataset(pseudo_items)
        logger.info("Pseudo labels on "+str(cfg.DATASETS.NAMES)+" statistics")
        pseudoDataset.show_train()
        train_set = CommMBDataset(pseudo_items, transforms, reliability = train_reliability, cam_style = cam_images, relabel=True)
        # train_set = CommDataset(pseudo_items, transforms, relabel=True)
